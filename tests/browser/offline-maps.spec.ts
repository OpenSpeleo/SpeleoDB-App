...
test('layer switches reuse the union worker plan and preserve satellite coverage', async ({ page }) => {
  ...
  await page.addInitScript(() => {
    const state = window as unknown as {
      offlinePlannerStarts: number;
      layerSwitchChanges: Array<{ id: string; checked: boolean }>;
    };
    state.offlinePlannerStarts = 0;
    state.layerSwitchChanges = [];
    document.addEventListener('ionChange', (event) => {
      const target = event.target;
      if (!(target instanceof HTMLElement)) return;
      const id = target.dataset.testid;
      if (!id?.startsWith('layer-toggle-')) return;
      state.layerSwitchChanges.push({ id, checked: (event as CustomEvent<{ checked: boolean }>).detail.checked });
    }, true);
  ...
  for (const toggle of [hill, dark, hill, hill]) {
    // Exercise the mobile touch path; Ionic suppresses mouse clicks following
    // its drag gesture. Do not wait for persistence between rapid switches.
    await toggle.tap();
    await expect(satelliteProgress).toContainText('100%');
  }
  expect(await page.evaluate(() => (window as unknown as {
    layerSwitchChanges: Array<{ id: string; checked: boolean }>;
  }).layerSwitchChanges)).toEqual([
    { id: 'layer-toggle-esri-world-hillshade', checked: true },
    { id: 'layer-toggle-esri-world-hillshade-dark', checked: true },
    { id: 'layer-toggle-esri-world-hillshade', checked: false },
    { id: 'layer-toggle-esri-world-hillshade', checked: true },
  ]);
