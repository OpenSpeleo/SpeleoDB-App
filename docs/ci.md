...
deployment targets all enforce 16.4 in Debug and Release, matching Vite's
`safari16.4` and `ios16.4` targets and the CSS framework's browser requirements.
The operating system rejects installation on older releases before app code
runs; no runtime compatibility shim or additional startup work is needed.

Capacitor generates `CapApp-SPM/Package.swift` using only the deployment
target's major version (`.v16`). That internal dependency package's floor is not
the app's installation requirement. Keep it generated by `cap sync ios`; the
app's packaged `MinimumOSVersion` must remain `16.4`.

The configuration contract test checks all native deployment declarations and
the actual Vite targets. Native verification must additionally build the app,
inspect its generated `Info.plist` minimum OS, and run signed simulator tests.
`AppDelegateTests` also asserts the installed test host app’s
`MinimumOSVersion`, so native verification exercises the packaged installation
requirement. Record the tested simulator runtime separately: a recent simulator
does not prove behavior on the oldest supported release.
