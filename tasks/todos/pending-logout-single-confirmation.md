...
with pending offline operations must see one irreversible-loss confirmation;
the generic local-data confirmation is reserved for users with no pending
operations.
...
- Red: `npm run test.unit -- --run src/pages/Settings.test.tsx -t "requires explicit acknowledgement before losing one pending offline operation"`
