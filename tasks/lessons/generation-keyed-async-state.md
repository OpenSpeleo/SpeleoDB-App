...
data only when its generation matches current inputs. Async callbacks merge
into the matching generation or start a new one, and the effect cleanup still
rejects stale completion. This hides prior data synchronously without an
effect-driven reset render.
